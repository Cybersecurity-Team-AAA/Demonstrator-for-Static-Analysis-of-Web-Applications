...
router.get("/search/seller", function(req, res){
    if ((req.session.user !== undefined && req.session.user.role === "user")) {  
        ...
        const finalSellerUsernameToSearch = braces(finalRegex, { expand: true });    // oops.... I put this in theroy to be able to search more seller at once, via a regex.... but this was a naive thing... braces is vuln
